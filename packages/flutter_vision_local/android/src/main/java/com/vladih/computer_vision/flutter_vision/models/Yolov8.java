...
public class Yolov8 extends Yolo {
    ...
    public List<Map<String, Object>> detect_task(ByteBuffer byteBuffer,
        ...
        try {
            ...
            // Model NCHW ([1, channels, height, width]) punya dims spasial di
            // posisi [2]/[3], bukan [1]/[2] seperti NHWC — kalau tidak
            // dibedakan, box decoding memakai channel count (mis. 3) sebagai
            // lebar/tinggi dan hasilnya rusak total.
            int inputHeight, inputWidth;
            if (input_shape.length == 4 && input_shape[1] <= 4 && input_shape[3] > 4) {
                inputHeight = input_shape[2];
                inputWidth = input_shape[3];
            } else {
            ...
            List<float[]> boxes = filter_box(this.output, iou_threshold, conf_threshold,
                    class_threshold, inputWidth, inputHeight);
            boxes = restore_size(boxes, inputWidth, inputHeight, source_width, source_height);
