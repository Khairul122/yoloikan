...
public class FlutterVisionPlugin implements FlutterPlugin, MethodCallHandler {
    ...
    private static class DetectionTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                if (shape.length == 3) {
                    ...
                    if (shape[1] <= 4 && shape[3] > 4) {
                        // Format: [batch, channels, height, width] (mis. model
                        // hasil export PyTorch/ONNX yang tidak dikonversi ke NHWC)
                        isNCHW = true;
                ...
                // FeedInputTensorHelper (TensorImage/ImageProcessor) selalu
                // menghasilkan buffer NHWC — untuk model NCHW, transpose dulu
                // sebelum dikirim ke interpreter, kalau tidak inferensi akan
                // salah total (channel & spatial dims tertukar).
                byteBuffer = utils.feedInputTensor(bitmap, inputWidth, inputHeight, srcWidth, srcHeight, 0, 255);
                if (isNCHW) {
                    byteBuffer = nhwcToNchw(byteBuffer, inputHeight, inputWidth, inputChannels);
                }
        ...
        /**
         * Transpose a float32 NHWC direct ByteBuffer (as produced by
         * FeedInputTensorHelper/TensorImage) into NCHW layout, for models
         * exported with a channels-first input tensor.
         */
        private static ByteBuffer nhwcToNchw(ByteBuffer nhwcBuffer, int height, int width, int channels) {
            nhwcBuffer.order(ByteOrder.nativeOrder());
            nhwcBuffer.rewind();
            FloatBuffer nhwc = nhwcBuffer.asFloatBuffer();

            ByteBuffer nchwBuffer = ByteBuffer
                    .allocateDirect(height * width * channels * 4)
                    .order(ByteOrder.nativeOrder());
            FloatBuffer nchw = nchwBuffer.asFloatBuffer();

            for (int c = 0; c < channels; c++) {
                for (int h = 0; h < height; h++) {
                    int rowBase = (h * width) * channels + c;
                    for (int w = 0; w < width; w++) {
                        nchw.put(nhwc.get(rowBase + w * channels));
                    }
                }
            }
            nchwBuffer.rewind();
